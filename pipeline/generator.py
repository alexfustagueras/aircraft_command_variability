    ...
    if "vz_sel_replay" not in commands.columns and vz_target_fpm.notna().any():
        ...
        if vz_fill_enabled(cfg):
            vz_target_fpm = pd.Series(
                fill_vz_sel(vz_target_fpm.to_numpy(dtype=float), **vz_fill_kwargs(cfg)),
...
def build_node_fdm_inputs(
    ...
    *,
    ...
    # Keep targets numerically finite even when the corresponding *_known mask is 0.
    # The predictor expects dense arrays and uses the known flags to gate these channels.
