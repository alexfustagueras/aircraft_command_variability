...
def _transition_to_vz_segments(
    h_from: float,
    h_to: float,
    tas_from_kt: float,
    tas_to_kt: float,
    ...
    *,
    ...
    """One (h_from → h_to) transition → list of segment dicts with phase / value / duration_s."""
    if tau_target_s <= 0:
        return []
    tau_from_ms = tas_from_kt * KT_TO_MS
    tau_to_ms = tas_to_kt * KT_TO_MS
    scaled = _rescale_power_segments(
        segments_tau_s=segments_tau_s,
        segments_p_eff_wkg=segments_p_eff_wkg,
        tau_target_s=tau_target_s,
        h_from=h_from,
        h_to=h_to,
        tas_from_ms=tau_from_ms,
        tas_to_ms=tau_to_ms,
    )
    if scaled is None:
        return []
    ...
    # Sample γ / vz per row by linearly interpolating H_E across segments and
    # using the energy identity.
    # We fill dt_s-resolution rows; for each row, compute p_rdp from the
    # neighbouring H_E boundary, vz from energy closure, gamma from vz/TAS.
    segments_t: list[float] = []
    cur = 0.0
    for t in new_tau:
        segments_t.append(cur)
        cur += float(t)
    segments_t.append(cur)
    total_t = float(segments_t[-1])
    if total_t <= 0:
        return []

    # Walk segment-by-segment so each row carries the correct duration. Avoids
    # discretisation error from linspace + uniform dt_s.
    rows_out: list[dict[str, Any]] = []
    t_cursor = 0.0
    for k, tau_seg in enumerate(new_tau):
        n_seg_rows = max(1, int(round(float(tau_seg) / dt_s)))
        dt_seg = float(tau_seg) / n_seg_rows
        for _ in range(n_seg_rows):
            tas_now = tau_from_ms + (tau_to_ms - tau_from_ms) * (t_cursor + dt_seg / 2.0) / total_t
            dTAS_dt_m_s2 = (tau_to_ms - tau_from_ms) / max(total_t, 1e-6)
            vz_m_s = (float(new_power[k]) - tas_now * dTAS_dt_m_s2) / G
            gamma_rad = float(energy_gamma_rad(vz_m_s, tas_now))
            vz_fpm = vz_m_s * 60.0 / FT_TO_M
            phase = "CLIMB" if h_to > h_from else "DESCENT"
            rows_out.append({
                "phase": phase,
                "command": "fdm_vz_target_fpm",
                "value": float(vz_fpm),
...
def replace_vz_with_rdp_segments(
    sampled: pd.DataFrame,
    *,
    transition_library: pd.DataFrame,
    tas_kt_at_h_from: dict[tuple[str, float], float],
    tas_kt_at_h_to: dict[tuple[str, float], float],
    rng: np.random.Generator,
    dt_s: float = 1.0,
    n_obs_default: float = 1.0,
) -> tuple[pd.DataFrame, list[dict[str, Any]]]:
    """Replace ``fdm_vz_target_fpm`` rows in ``sampled`` with RDP-derived vz.

    Walks consecutive ``fdm_alt_target_ft`` plateau pairs, samples one
    strictly matching transition row per pair from ``transition_library``,
    reconstructs γ, and emits dt_s-resolution vz rows. Keeps the
    ``fdm_alt_target_ft`` rows unchanged; replaces the
    ``fdm_vz_target_fpm`` rows.

    The transition library is the source of truth. Unsupported exact pairs
    raise ``LibraryTooSparse``; the connected public sampler avoids creating
    such pairs in the first place.

    Returns ``(commands, fallback_log)`` for compatibility. The log contains
    only ``"strict"`` entries and does not gate sampling.
    """
    sampled = sampled.copy()
    phase = str(sampled["phase"].iloc[0]).upper() if not sampled.empty else ""

    h_seg = sampled.loc[sampled["command"] == "fdm_alt_target_ft"].copy()
    if len(h_seg) < 2:
        return sampled, []
    ...
    prev_h = None
    for _, row in h_seg.reset_index(drop=True).iterrows():
        h = float(row["value"])
        if prev_h is None:
            out_rows.append(row.to_dict())
            prev_h = h
            continue
        if phase == "CLIMB" and h <= prev_h:
            out_rows.append(row.to_dict())
            prev_h = h
            continue
        if phase == "DESCENT" and h >= prev_h:
            out_rows.append(row.to_dict())
            prev_h = h
            continue

        regime = "MACH" if h >= 28000.0 else "CAS"
        h_from = float(round(prev_h / 500.0) * 500.0)
        h_to = float(round(h / 500.0) * 500.0)
        tau_target_s = float(row["duration_s"])
        if abs(h_to - h_from) < 500.0 or tau_target_s <= 0:
            out_rows.append(row.to_dict())
            prev_h = h
            continue

        trans_row, fallback_level = sample_transition_row(
            transition_library,
            phase=phase,
            h_from=h_from,
            h_to=h_to,
        ...
        fallback_log.append({
            ...
            "n_obs": int(trans_row.get("n_segments", 0)) if isinstance(trans_row, pd.Series) else 0,
        })

        tas_from_kt = tas_kt_at_h_from.get((phase, prev_h), tas_kt_at_h_from.get(("default", 0.0), 250.0))
        tas_to_kt = tas_kt_at_h_to.get((phase, h), tas_kt_at_h_to.get(("default", 0.0), 280.0))

        vz_rows = _transition_to_vz_segments(
            h_from=h_from,
            h_to=h_to,
            tas_from_kt=tas_from_kt,
            tas_to_kt=tas_to_kt,
            tau_target_s=tau_target_s,
            segments_tau_s=list(trans_row["segments_tau_s"]),
            segments_p_eff_wkg=list(trans_row["segments_p_eff_wkg"]),
            speed_regime=regime,
            dt_s=dt_s,
        )
        out_rows.append(row.to_dict())
        out_rows.extend(vz_rows)
        prev_h = h

    new_df = pd.DataFrame(out_rows, columns=sampled.columns)
    if "speed_regime" not in new_df.columns:
        new_df["speed_regime"] = "CAS"
    return new_df, fallback_log
